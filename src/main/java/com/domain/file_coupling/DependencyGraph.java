...
public class DependencyGraph extends Graph<FileTechnologyStack, Double> {
  public DependencyGraph(
    ...
    Collection<FileTechnologyStack> filesWithoutGit = files.stream()
            .filter(f -> !f.file()
                    .getName()
                    .equals(".git"))
            .toList();
    if (repoPath == null) {
      throw new IllegalStateException("Git directory not presented in " +
              "the " +
              "input");
    }
    Repository repo = new RepositoryBuilder().setGitDir(repoPath.file())
            .build();
    // Create FileGraph and calculate dependencies
    FileGraph fileGraph = new FileGraph(
            filesWithoutGit, "src/main/resources" +
            "/application.yaml", repo
    );
    ...
    for (Graph.EdgeInfo<FileGraph.Vertex, FileGraph.Edge> edgeInfo : fileGraph.getEdges()) {
      ...
      if (source != null && destination != null) {
        addEdge(
                source, destination, edgeInfo.edgeData()
                        .getCompoundWeight()
        );
      }
    }
